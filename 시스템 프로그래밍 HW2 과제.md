...
```
`myprintf()`함수에서 `%d`가 포함되면 실행되는 함수입니다. 가변 인자의 값을 매개변수로 받아와서 매개변수의 값을 출력합니다.
int를 `putchar`함수를 이용해 하나씩 끊어 출력해야 하므로 매개변수인 value값을 10 단위로 끊어줍니다. 10단위로 끊어주게 되면 buffer에는 1의 자리의 수부터 큰 자리의 수까지 값이 거꾸로 저장되게 됩니다. 따라서 다시 while문을 이용하여 `while(--i >= 0)`을 이용해 buffer를 거꾸로 읽어들이면서 `putchar`를 이용해 int 값을 전부 출력해줍니다.
...
```
`myprintf()`함수에서 `%s`가 포함되면 실행되는 함수입니다. 가변 인자의 값을 매개변수로 받아와서 매개변수의 값을 출력합니다.
