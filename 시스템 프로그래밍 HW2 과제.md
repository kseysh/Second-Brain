...
`printf.h`파일을 생성하여 라이브러리에 있는 `printf.h`가 아니라 제가 생성한 `printf.h`파일을 사용하도록 해줍니다. `#define`을 통해 `printf`함수를 `myprintf`함수로 interposition 하겠다는 것을 정의하고, `myprintf()`함수를 헤더파일에 정의해줍니다.
이를 통해 컴파일러는 printf를 사용할 때 `printf`함수 대신 `myprintf`를 사용하게 됩니다.
### `gcc -DCOMPILETIME -I. -o hw2C main.c myprintf.c`
위 명령어를 통해 `main.c`와 `myprintf.c`파일을 함께 컴파일하여 `hw2C` 실행파일을 만들게 됩니다.
###### `-DCOMPILETIME`
-DCOMPILETIME 옵션은 define COMPILETIME의 약자로 COMPILETIME  매크로를 정의하고 이로 인해서 `#ifdef COMPILETIME` 과 `#endif` 안의 코드를 활성화 할 수 있도록 합니다. 따라서 이 코드는 컴파일 시간에만 포함되는 코드가 됩니다.
###### `-I.`
`-I.`옵션은 현재 디렉토리에 있는 헤더 파일(`printf.h`)로 Interpositioning을 하겠다는 옵션으로,  `.`으로 작성하여 현재 디렉토리를 나타냅니다.
현재 디렉토리를 include 파일의 검색 경로로 지정할 수 있도록 하였습니다.
###### `-o`
