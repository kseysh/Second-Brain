...
###### Keepalive Timer란?
오랜 기간 동안 idle 상태에 있는 것을 방지하기 위해 클라이언트가 하드웨어적으로 죽으면 FIN을 보내지 못하고 죽어버림 이를 서버가 계속 가지고 있지 않기 위해 2시간 동안 가지고 있다가 세그먼트를 전송함.
...
Original transmission이 사라졌는지, Retransmission이 사라졌는지 sender는 구별할 수 없기 때문에 재전송에 대한 measurement값이 오류가 발생할 수 있기 때문이다.
...
cumulative ack의 단점인 packet loss가
생겼을 때 detail한 상황을 말할 수 없다는 단점을 보완하기 위해서
###### class 구분하기
