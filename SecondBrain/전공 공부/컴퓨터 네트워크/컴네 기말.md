...
여러 개의 IP 주소를 하나의 큰 네트워크로 통합하여 라우팅 테이블의 크기를 줄이고 네트워크 효율성을 향상시키는 기술
...
M은 분할 후 재조합할 때 마지막 패킷이 뭔지를 찾아보기 위해 사용하는 field
###### HLEN과 offset 계산법
HLEN x 4
offset x 8
###### MTU란?
...
![[Pasted image 20241212172846.png|400]]
###### Distance vector routing 하는 방법
![[Pasted image 20241212173109.png|400]]
• 각 노드는 자신과 직접 연결된 노드들의 정보만을 가지고 초기 라우팅 테이블을 만든다.
• 이웃 노드로부터 테이블을 받아, 자신의 라우팅 테이블과 더해 임시 테이블을 만들고, 기존 테이블을 업데이트한다.
	• 이웃 노드(next)가 다를 경우, 비용이 작은 경로로 업데이트합니다.
	• 이웃 노드(next)가 같은 경우, 새로운 정보로 기존 정보를 업데이트합니다.
• 이웃 노드들과 지속적으로 정보 교환, 테이블 업데이트 반복.
###### Two node instablility 상황 설명
![[Pasted image 20241203222643.png|350]]
X, A, B 라우터가 순서대로 연결되어 있을 때, X 라우터에서 A로 가는 연결이 끊어져서 A 라우팅 테이블이 업데이트 되고, B가 다시 A에게 라우팅 테이블을 보내면, next가 다른 상황의 라우팅 정보를 보내는 것이므로 라우팅테이블이 업데이트 된다.
그러면 A도 테이블이 업데이트 되어 다시 B에게 정보를 보내고 이 상황이 반복된다.
...
X-A가 끊어져 A가 B와 C에게 정보를 전송하는데, C로 가는 패킷이 없어진 상황
이 때 C가 주기적으로 정보를 보내게 되면, B는 next가 다르고 cost가 더 작은 C가 보낸 라우팅 정보를 택한다.
B는 A에게 라우팅 정보를 보내고 A도 next가 다르고 cost가 더 작은 B가 보낸 라우팅 정보를 택한다.
...
무슨 에러가 생겼는지, 어디서 에러가 생겼는지, 어디로 가려했는지, 어떤 프로그램에서 오류가 발생했는지
