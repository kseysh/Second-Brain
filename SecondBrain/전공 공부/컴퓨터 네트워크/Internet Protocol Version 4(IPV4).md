...
### ex 3
UNIX에서 IP 패킷의 이동을 확인하기 위해 사용할 수 있는 유틸리티 중 하나는 ping입니다. 다음 장에서는 ping 프로그램에 대해 더 자세히 설명합니다. 이 예제에서는 호스트가 사용 가능한지 확인하기 위해 프로그램을 사용하는 방법을 보여줍니다. De Anza College의 서버 fhda.edu에 ping을 보냅니다. 결과는 호스트의 IP 주소가 153.18.8.1임을 보여줍니다. 결과는 또한 사용된 바이트 수를 보여줍니다.
![[Pasted image 20241127151942.png|400]]
### ex 4
ping 유틸리티는 -R 옵션과 함께 사용하여 경로 기록 옵션을 구현할 수도 있습니다. 결과는 인터페이스와 IP 주소를 보여줍니다.
traceroute 유틸리티는 패킷의 경로를 추적하는 데에도 사용할 수 있습니다. 결과는 방문한 세 개의 라우터를 보여줍니다.
![[Pasted image 20241127152011.png|400]]
### ex5
traceroute 프로그램은 느슨한 소스 라우팅을 구현하는 데 사용할 수 있습니다. -g 옵션을 사용하면 출발지에서 목적지까지 방문할 라우터를 정의할 수 있습니다. 다음은 패킷이 라우터 153.18.251.4를 방문하도록 요구하면서 fhda.edu 서버로 패킷을 보내는 방법을 보여줍니다.
![[Pasted image 20241127152050.png|400]]
### ex6
traceroute 프로그램은 엄격한 소스 라우팅을 구현하는 데에도 사용할 수 있습니다. -G 옵션은 명령줄에 정의된 라우터만을 방문하도록 패킷을 강제합니다. 다음은 패킷이 오직 라우터 153.18.251.4만 방문하도록 강제하면서 fhda.edu 서버로 패킷을 보내는 방법을 보여줍니다.
![[Pasted image 20241127152039.png|400]]
## checksum
그닥 안 중요한 듯
## IP components
