...
###### Buffer의 세 가지 영역과 sent 영역이 필요한 이유 not sent -> sent -> empty -> not sent로 변하는 과정
sent, not sent, empty
...
not sent -> sent 데이터를 보내면 sent로 변함
sent -> empty 데이터를 받았다고 ACK을 받으면 변경
empty -> not sent application이 데이터를 준비해서 넣어두면 변함

###### Boundary delivary란?
UDP가 사용하는 방식으로 message에서 만들어진 boundary가 전달이 될 때 유지된다.
###### Selective ACK과 Cumulative ACK의 차이, Cumulative ACK의 장단점
...
Fast retransmit을 수비게 구현하여 packet lost를 쉽게 감지할 수 있다,
Receiver의 ACK이 lost되어도 Resent하지 않아도 되는 Case가 있다.
단점
중간의 패킷이 오지 않은 것인지, 천천히 가고 있는 것인지 몰라 packet loss 상황을 자세히 알 수 없다.
###### HLEN의 역할과 특징
...
###### window size가 필요한 이유
SYN을 보내고, 기다렸다가 ACK을 받고 다시 SYN을 보내는 STOP&WAIT방식에는 시간이 너무 많이 든다.
따라서 한 번에 SYN을 n만큼 보내기 위해서, 받는 쪽에서 받을 수 있는 패킷의 수를 고려하기 위함이다.
...
###### 확인 응답의 지연에 대해서 설명
수신 측에서 발생하는 지연 해결책으로 버퍼에 일정 크기 이상의 빈공간이 생겼을 때 ACK을 보내고, 아이면 보내지 않는다.
###### SYN Flooding이란?
서버에게 SYN을 보내고 서버는 Client에게 SYN/ACK을 보내고 연결 요청 대기 큐에 대기시켜 놓는데, 요청이 많거나 악의적으로 ACK을 받을 수 없는 SYN만 보내면 연결 요청 대기 큐가 가득 차서 유저가 연결이 안되는 상황이다.
#### Rule 1,2,3를 설명하시오
![[Pasted image 20241010201504.png|300]]
###### 정답
- Rule 1
...
#### Rule 4,5,6를 설명하시오
![[Pasted image 20241011164502.png|300]]
![[Pasted image 20241011172027.png|300]]
###### 정답
- Rule 4
...
###### Fast retransmission에 대해서 설명하시오
...
Sender가 Receiver에게 rwnd = 0 을 받아 대기하고, Receiver는 Sender에게 rwnd = k라는 ACK을 보냈지만 유실되어 Sender가 데이터를 보내주기를 기다리는 상황에 DeadLock이 발생할 수 있다.
