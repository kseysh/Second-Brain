...
## 5-6 클럭 S-R 플립플롭
- S와 R 입력은 클럭 펄스가 발생할 때 플립플롭(FF)이 어떤 상태로 변할지를 제어하는 동기 제어 입력입니다.
	- CLK 입력은 S와 R 입력에 따라 플립플롭이 상태를 변경하도록 하는 트리거 입력입니다.
- SET-RESET(또는 SET-CLEAR) 플립플롭은 양의 변환 엣지 또는 음의 변환 엣지에서 상태를 변경합니다.

클럭 신호의 양의 변환 엣지에서 트리거되는 클럭 S-R 플립플롭.
![[Pasted image 20241106190854.png|500]]
S와 R 입력은 앞서 설명한 NOR-NOR 게이트 래치와 동일한 방식으로 플립플롭의 상태를 제어하지만, 플립플롭은 클럭 신호의 양의 변환 엣지가 발생할 때까지 이러한 입력에 응답하지 않습니다.


클럭 펄스의 양의 변환 엣지에서 트리거되는 클럭 S-R 플립플롭의 동작 파형.
![[Pasted image 20241106190939.png|300]]

클럭 신호의 음의 변환 엣지에서 트리거되는 클럭 S-R 플립플롭.


디지털 시스템에서는 양의 변환 엣지 트리거 플립플롭과 음의 변환 엣지 트리거 플립플롭이 모두 사용됩니다.
...
*J*와 *K* 입력이 *CLK* 전이와 거의 동시에 변경되는 경우가 있다.
