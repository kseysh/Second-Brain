...
- 컴퓨터가 해석하는 언어는 엄격한 문법 규칙을 따라야 합니다.
    - 문법(Syntax)은 요소들의 순서를 의미합니다. 다이어그램의 왼쪽에는 입력 세트가 있고, 오른쪽에는 출력 세트가 있습니다. 

- 형식(Format)은 입력, 출력 및 출력이 입력에 반응하는 방식을 정의합니다(작동). 

- 텍스트 기반 언어에서, 설명된 회로는 이름을 가져야 합니다.
    - 작동의 정의는 입력/출력(I/O) 정의에 이어지는 일련의 문장에 포함됩니다.
- 입력 및 출력(포트)은 이름이 지정되고 포트의 특성에 따라 정의되어야 합니다.
    - 모드(mode)는 입력인지 출력인지 또는 양쪽인지 정의합니다.
    
- 타입(type)은 비트 수와 그 비트들이 어떻게 그룹화되고 해석되는지를 나타냅니다.
	- 단일 비트 입력은 0과 1 두 가지 값만 가질 수 있습니다.
    - 4비트 이진수는 16가지 값(0000₂ - 1111₂) 중 하나를 가질 수 있습니다. 
 
- 키워드 **SUBDESIGN**은 회로 블록에 이름을 부여하며, 이 경우는 `and_gate`입니다. 파일 이름은 반드시 `and_gate.tdf`이어야 합니다. 입력 변수를 쉼표로 구분하고 뒤에 `:INPUT`를 붙입니다. AHDL에서 입력/출력 정의는 괄호 안에 포함됩니다. AHDL에서는 단일 비트 타입이 기본값으로 가정되며, 변수가 여러 비트로 지정되지 않는 한 단일 비트로 간주됩니다. 단일 출력 비트는 `:OUTPUT` 모드로 선언됩니다.
