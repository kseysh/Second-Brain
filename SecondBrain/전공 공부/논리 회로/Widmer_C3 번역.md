...
# 3-18 PLD를 사용한 논리 회로 구현
- PLD(Programmable Logic Devices, 프로그래머블 논리 소자)는 여러 방식으로 구성되어 논리 기능을 수행할 수 있는 장치입니다.
    - 내부 연결은 전자적으로 이루어져 소자를 프로그램합니다. PLD는 전자적으로 구성되며, 내부 회로는 전자적으로 "배선"되어 논리 회로를 형성합니다. 이러한 프로그래머블 배선은 수천 개의 연결을, 연결됨(1) 또는 연결되지 않음(0)으로 간주할 수 있습니다. 가로선(수평선)과 세로선(수직선)의 교차점 각각은 프로그래머블 연결입니다. 하드웨어 설명 언어는 이루어질 연결을 정의합니다.
    - 컴파일러에 의해 변환된 후 소자에 로드됩니다.
- 상위 레벨 하드웨어 설명 언어는 부울 대수, 회로도, 진리표를 사용하는 것보다 PLD를 프로그래밍하는 것을 훨씬 쉽게 만듭니다.

# 3-19 HDL 형식과 문법
- 컴퓨터가 해석하는 언어는 엄격한 문법 규칙을 따라야 합니다.
    - 문법(Syntax)은 요소들의 순서를 의미합니다. 다이어그램의 왼쪽에는 입력 세트가 있고, 오른쪽에는 출력 세트가 있습니다. 형식(Format)은 입력, 출력 및 출력이 입력에 반응하는 방식을 정의합니다(작동). 텍스트 기반 언어에서, 설명된 회로는 이름을 가져야 합니다.
    - 작동의 정의는 입력/출력(I/O) 정의에 이어지는 일련의 문장에 포함됩니다.
- 입력 및 출력(포트)은 이름이 지정되고 포트의 특성에 따라 정의되어야 합니다.
    - 모드(mode)는 입력인지 출력인지 또는 양쪽인지 정의합니다. 타입(type)은 비트 수와 그 비트들이 어떻게 그룹화되고 해석되는지를 나타냅니다.
    - 단일 비트 입력은 0과 1 두 가지 값만 가질 수 있습니다.
    - 4비트 이진수는 16가지 값(0000₂ - 1111₂) 중 하나를 가질 수 있습니다. 키워드 **SUBDESIGN**은 회로 블록에 이름을 부여하며, 이 경우는 `and_gate`입니다. 파일 이름은 반드시 `and_gate.tdf`이어야 합니다. 입력 변수를 쉼표로 구분하고 뒤에 `:INPUT`를 붙입니다. AHDL에서 입력/출력 정의는 괄호 안에 포함됩니다. AHDL에서는 단일 비트 타입이 기본값으로 가정되며, 변수가 여러 비트로 지정되지 않는 한 단일 비트로 간주됩니다. 단일 출력 비트는 `:OUTPUT` 모드로 선언됩니다.
- AHDL 회로의 작동을 설명하는 문장은 **BEGIN**과 **END** 키워드 사이의 논리 섹션에 포함됩니다. **END** 뒤에는 세미콜론이 붙어야 합니다. **BEGIN**과 **END** 사이의 문장은 끊임없이 동시에 평가되며, 나열된 순서는 중요하지 않습니다.
- **ENTITY** 키워드는 회로 블록에 이름을 부여하며, 이 경우는 `and_gate`입니다. 컴파일러가 지정한 변수는 소문자여야 합니다. **PORT** 키워드는 이 회로 블록에 입력과 출력을 정의하고 있음을 컴파일러에게 알립니다.
- **BIT** 설명은 컴파일러에게 목록에 있는 각 변수가 단일 비트임을 알려줍니다. **ARCHITECTURE** 선언은 블록 내부의 모든 동작을 설명하는 데 사용됩니다. 모든 **ENTITY**에는 최소한 하나의 **ARCHITECTURE**가 연관되어 있어야 합니다. **BEGIN**과 **END** 사이의 본문에서는 블록의 작동이 설명됩니다.
# 3-20 중간 신호
- 많은 설계에서 회로 블록 "내부"에 신호 지점을 정의해야 할 필요가 있습니다. 이를 **매립 노드(buried nodes)** 또는 **지역 신호(local signals)**라고 합니다.
    - 입력이 아닌, 회로 내에서 참조점으로 유용할 수 있는 지점입니다.
**AHDL 지역 신호:**
- 주석은 `%` 문자로 둘러싸여 있습니다.
- 두 개의 대시(--) 뒤에 오는 텍스트는 문서용입니다.
- 키워드 **VARIABLE**은 중간 신호를 정의합니다.
- 키워드 **NODE**는 변수의 성질을 나타냅니다.
**VHDL 지역 신호:**
- 두 개의 대시(--) 뒤에 오는 텍스트는 문서용입니다.
- 키워드 **SIGNAL**은 중간 신호를 정의합니다.
- 키워드 **BIT**는 신호의 타입을 지정합니다.