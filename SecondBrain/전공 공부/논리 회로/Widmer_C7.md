...
- 주파수 분할 - 각 FF는 입력의 1/2 주파수의 출력을 가진다.
...
- 입력 펄스 사이의 주기가 카운터의 총 전파 지연보다 길면 문제가 발생하지 않는다.
  - 적절한 작동 조건: \( T_{clock} \geq N \times t_{pd} \)
  - 최대 주파수: \( F_{max} = \frac{1}{N \times t_{pd}} \)
...
### 7-4 MOD 번호가 \( 2^N \) 미만인 카운터
- 기본 동기 카운터는 \( 2^N \)과 같은 MOD 번호로 제한된다.
  - 여기서 N은 FF의 수이다.
...
- 동기형 MOD-16 다운 카운터와 출력 파형:
  ...
  - 각 상태의 버블에서 나오는 두 개의 화살표는 조건부 전이를 나타낸다. 다음 상태는 제어 입력에 적용된 논리 레벨에 따라 결정된다.
...
### 7-8 카운터 디코딩
- 디지털 카운터는 FF 상태로 표현된 카운트를 판독하거나 표시해야 하는 응용에서 종종 사용된다.
  - 카운터의 내용을 표시하는 가장 간단한 방법 중 하나는 FF 출력을 LED에 연결하는 것이다.
- 디코딩은 이진 출력을 10진 값으로 변환하여 즉시 인식할 수 있도록 하는 과정이다.
  - 예시로, 0부터 7까지 각 10진수를 나타내는 LED를 점등하는 활성-HIGH 디코더가 있다.
