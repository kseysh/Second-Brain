...
`WCONTINUED`: 구현이 작업 제어를 지원하는 경우, 중지된 후 계속된 자식의 상태가 아직 보고되지 않은 경우 상태가 반환됩니다 (POSIX.1의 XSI 확장).
`WNOHANG`: pid로 지정된 자식이 즉시 사용 가능하지 않으면 waitpid 함수는 블록하지 않습니다. 이 경우 반환 값은 0입니다. (hang = 매달다 = 기다리다 NOHANG = 기다리지 말라)
`WUNTRACED`: 구현이 작업 제어를 지원하는 경우, 중지된 이후 보고되지 않은 자식의 상태가 반환됩니다. WIFSTOPPED 매크로는 반환 값이 중지된 자식 프로세스에 해당하는지 여부를 결정합니다.
