...
- 디렉토리에 대한 읽기 권한과 실행 권한은 다른 의미를 가집니다.
  ...
  - **실행 권한**: 디렉토리를 통과하여 특정 파일명을 검색할 수 있습니다.
- 따라서 디렉토리에 대한 실행 권한 비트는 종종 검색 비트라고도 합니다.
- **저장-텍스트-이미지(sticky bit) - S_ISVTX**
  - 이 비트가 디렉토리에 설정된 경우:
    - 사용자는 디렉토리에 대한 쓰기 권한과 다음 중 하나를 만족해야만 파일을 제거하거나 이름을 변경할 수 있습니다:
      - 파일 소유자
      - 디렉토리 소유자
      - 슈퍼유저
...
- 더 중요한 것은, mkdir은 새로 생성된 디렉토리에 두 개의 링크 `.`와 `..`를 추가한다는 점입니다.
- 일반 파일과 동일한 모드를 지정하는 실수를 저지르는 경우가 많습니다: 읽기 및 쓰기 권한만. 하지만 디렉토리의 경우, 디렉토리 내 파일명에 접근할 수 있도록 적어도 하나의 실행 비트가 활성화되기를 원합니다.
