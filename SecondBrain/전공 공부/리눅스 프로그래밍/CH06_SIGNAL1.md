...
- *만약 프로세스가 "느린" 시스템 호출 동안 신호를 잡으면, 시스템 호출이 중단됩니다*.
  - 시스템 호출이 오류를 반환하고, 오류 번호(`errno`)가 `EINTR`로 설정됩니다.(system call은 kernel에서 실행되고, user process가 중단을 시키므로 kernel과 user process는 다른 process이므로 kernel에서 실행하던 부분으로 다시 돌아갈 수 없기 때문)
- 시스템 호출은 두 가지로 나뉩니다:
