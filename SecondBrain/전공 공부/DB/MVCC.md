...
### postgreSQL
=> 같은 데이터에 먼저 update한 tx가 commit되면 이후 tx는 rollback된다. (두 개의 tx가 같은 데이터를 수정했을 때만 롤백 되는 것)
...
postgreSQL과 다르게 Locking read를 이용해 repeatable read에서의 Lost Update를 해결한다.
