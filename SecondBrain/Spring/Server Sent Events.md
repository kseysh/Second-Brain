...
SSE보다 초기 연결 비용이 크지만, HTTP Header가 존재하지 않아 간결하다. (프레임 오버헤드가 작다 but 헤더 많이 전송안함)
# Server-Sent Events
...
HTTP 요청이므로 모든 네트워크 환경에서 100% 동작
하나의 TCP 연결을 통해 여러 SSE 스트림을 동시에 받을 수 있음
재접속은 브라우저가 자동 처리한다.
데이터 복구 로직
단방향 알림/이벤트 스트림에는 HTTP 생태계와 잘 맞는 SSE가 운영상 유리하다.
