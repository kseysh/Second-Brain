멋쟁이 사자처럼 해커톤에서 소셜로그인 api를 만들고 회원가입을 어떻게 처리할지 고민하다가 JWT를 써보기로 했습니다.  session_id를 이용한 로그인 방식은 저번 해커톤에서도 사용해보기도 하였으며, JWT쪽이 보안을 더 강화할 수 있다는 말에 JWT방식으로 회원가입 및 로그인 로직을 구현해보고 싶어졌습니다.
