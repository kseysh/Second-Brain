...
G에 Hamiltonian Cycle이 존재하면 G'에 **가중치 1의 간선만으로 구성된 순회가 존재하므로 비용은 정확히 |V|가 되고**, TSP는 “yes”가 된다. 반대로 TSP가 “yes”라면 비용이 |V| 이하의 순회가 존재하므로, 이는 곧 Hamiltonian Cycle의 존재를 의미한다.
