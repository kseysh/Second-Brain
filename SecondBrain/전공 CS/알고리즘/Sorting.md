...
## Merge Sort
문제 
감소하지 않는 순서로 정렬된 두 개의 시퀀스 A와 B가 주어지면, 그것들을 병합하여 하나의 정렬된 시퀀스를 만듭니다. C 전략 C의 첫 번째 항목을 결정하십시오: A와 B의 첫 번째 항목 사이의 최소값입니다. 그것이 A의 첫 번째 항목이라고 가정해 봅시다. 그러면, C의 나머지 부분은 A의 나머지 부분을 B와 병합한 결과여야 한다.