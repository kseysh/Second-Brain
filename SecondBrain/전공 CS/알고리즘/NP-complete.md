...
TSP가 NP-Hard임을 보이기 위해, 이미 NP-완전임이 알려진 Hamiltonian Cycle 문제로부터 다항 시간 환원을 수행한다.
	•	Hamiltonian Cycle 문제는 무가중치 그래프 G가 주어졌을 때, 모든 정점을 정확히 한 번씩 방문하는 순회가 존재하는지를 묻는다.
	•	이를 TSP 결정 문제로 환원하기 위해, G를 기반으로 한 완전 가중치 그래프 G′를 만든다.
	•	G에 존재하는 간선에는 가중치 1을 부여하고,
	•	존재하지 않는 간선에는 가중치 ∞를 부여한다.
	•	이후 TSP 결정 문제의 상한값 K를 |V|로 설정한다.
	•	Hamiltonian Cycle이 존재하면 가중치 1의 간선만으로 구성된 순회가 존재하므로 비용은 정확히 |V|가 되고, TSP 결정 문제는 “yes”가 된다.
	•	반대로 TSP 결정 문제가 “yes”라면 비용이 |V| 이하의 순회가 존재하므로, 이는 곧 Hamiltonian Cycle의 존재를 의미한다.
