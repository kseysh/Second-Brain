...
select 함수는 운영체제의 커널의 기능이 아닌, 순수하게 응용프로그램 함수에 의해 완성되는 기능이므로 select 함수의 호출을 통해서 전달된 정보는 운영체제에 등록되지 않는다.
그래서 select 함수를 호출할 때마다 관련 정보를 운영체제에게 전달해야 하는 것이 select 함수의 단점의 원인이다.
하지만 이 단점으로 인해 호환성이 좋다는 장점이 있다.

epoll은 OS 레벨에서 멀티플렉싱 기능을 지원한다.
