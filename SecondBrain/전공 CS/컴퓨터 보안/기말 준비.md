...
•	Authenticate then encrypt: CCM
•	encrypt then Authenticate: GCM (Galois CTR 모드)
...
DH
Elgamal
Elliptic Curve 암호는 Discrete log와 구조상으로 비슷 사용은 X
###### DH key 교환
...
하지만, DH는 XA를 일회용으로 사용하기 때문에 안전하다
...
###### CA가 폐기되었으나 아직 만료되지 않은 인증서 목록(CRL)을 유지해야 하는 이유
인증서가 아래와 같은 이유로 만료 전 폐기가 필요할 수 있기 때문
...
- 모든 참여자들은 이전 트랜잭션에 포함된 공개키를 이용하여 이 서명을 확인함으로써 트랜잭션 유효성 검증
...
학습 시점 공격: Data poisoning / backdoor attack ⇒ Data sanitization
