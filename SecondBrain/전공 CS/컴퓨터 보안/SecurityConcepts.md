...
1. Fail-safe Defaults (실패 안전 기본값)
	•	접근 제어는 허용이 아닌 거부를 기본값으로 설정해야 함
	•	기본적으로 접근이 불가능하며, 특정 조건을 충족할 때만 접근을 허용
	•	대부분의 파일 접근 시스템 및 클라이언트/서버 보호 서비스에서 적용됨
	•	예: Linux Iptables에서 ACCEPT와 DROP 정책
2. Isolation (격리)
	•	공개 접근 시스템은 중요 자원과 분리되어야 함 (정보 유출 및 변조 방지)
	•	개별 사용자의 프로세스 및 파일은 명확하게 격리되어야 함
	•	보안 메커니즘 자체도 외부 접근으로부터 격리되어야 함
...
1. 위협(Threat)
	•	무단 접근, 파괴, 공개, 수정, 서비스 거부(DoS) 등을 통해 조직 운영(미션, 기능, 이미지, 평판 등)에 악영향을 미칠 수 있는 모든 상황 또는 사건
2. 공격(Attack)
