...
###### 4-way-handshake
TCP 연결을 해제하는 단계로 set up 단계와 달리 3-way-handshake가 아닌 이유는 Half-Close를 하기 위함입니다. 종료 요청은 보통 클라이언트 쪽에서 요청합니다. 이 때, 서버쪽에서 아직 전송해야 할 데이터가 남아있다면 클라이언트의 연결 종료 요청인 SYN에게 ACK을 보내고, 데이터 전송을 마무리 한 후 클라이언트에게 SYN을 보내기 위함입니다.
