...
###### EX data hazard 확인법
R-format의 rd가 다음이나 다다음의 rs, rt와 같으면 EX data hazard임 (하지만 forwarding으로 해결간으)
...
write allocation: write miss일 때 fetch한다
write around: write miss일 때 fetch하지 않는다.
###### write-back의 경우 
...
###### hamming distance와 minimum distance란?
• 해밍 거리
...
###### M-bit data일 때 SEC를 만들기 위한 Parity bit K
