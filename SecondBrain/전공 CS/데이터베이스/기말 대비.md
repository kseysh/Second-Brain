...
해당 블록에서 가장 작거나 같은 검색 키 값에 대응하는, 파일의 각 블록마다 하나의 인덱스 항목을 가지는 Sparse Index
=> 어차피 DB는 block단위로 data를 가져오기 때문
###### dense index와 sparse index의 블록 단위 접근 시간 복잡도
...
###### Range Search 시간 복잡도
O(log<sub>B<sub>index</sub></sub>N<sub>data</sub> + T/B<sub>index</sub>)
###### leaf node와 non leaf node의 분할 차이
leaf node는 value를 기준으로 `⌈B/2⌉`개와 나머지로 분할
non-leaf node는 ptr을 기준으로 `⌈(B+1)/2⌉`개와 나머지로 분할
###### Q
A
###### Q
A
