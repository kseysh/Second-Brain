...
•	등가 조인 및 자연 조인에 사용 가능.
•	해시 함수 h를 이용해 두 릴레이션의 튜플들을 파티션한다.
•	h는 조인 속성(JoinAttrs)의 값을 {0, 1, …, H}로 매핑한다.
	•	r의 튜플 tr은 h(tr[JoinAttrs]) = i 에 따라 ri에 할당
	•	s의 튜플 ts는 h(ts[JoinAttrs]) = i 에 따라 si에 할당

⇒ ri의 튜플은 오직 si의 튜플과만 비교하면 된다.
조인 조건을 만족하는 r, s 튜플은 조인 속성 값이 같고, 이는 같은 파티션으로 해시되기 때문이다.

⸻

Hash-Join 알고리즘
r과 s의 해시 조인은 다음 단계로 수행된다:
	1.	릴레이션 s를 해시 함수 h로 파티션한다.
	•	각 파티션마다 출력 버퍼로 메모리 블록을 하나 예약함
	2.	릴레이션 r도 같은 방식으로 파티션한다.
	3.	각 i에 대해 다음을 수행:
a. si를 메모리에 적재하고, 조인 속성으로 인메모리 해시 인덱스를 생성한다.
	•	이 인덱스는 h와는 다른 해시 함수를 사용함
b. ri의 튜플을 디스크에서 하나씩 읽어들인다.
	•	각 튜플 tr에 대해 si에서 일치하는 튜플 ts를 인메모리 해시 인덱스로 찾는다.
	•	속성들을 연결(concatenate)하여 결과로 출력한다.
...
추가 세부사항
	•	해시 함수 h와 파티션 수 H는 각 si가 메모리에 적재 가능하도록 설정해야 한다.
	•	일반적으로 H = Θ(Ns / M)으로 설정
	•	프로브 릴레이션(r)의 파티션은 메모리에 적재되지 않아도 됨

재귀적 파티셔닝(Recursive Partitioning)
	•	만약 파티션 수 H가 메모리 페이지 수 M/B보다 크면, 재귀적 파티셔닝 필요
	•	예: H-way 파티셔닝 대신, s를 M/B – 1개의 파티션으로 나누고, 이를 다시 다른 해시 함수로 추가 파티셔닝
	•	r도 동일 방식으로 파티셔닝

※ 실제로는 거의 필요 없음
	•	예: 블록 크기 4KB, 메모리 2MB인 경우 1GB 이하의 릴레이션은 재귀 파티셔닝이 필요 없음
	•	메모리 12MB인 경우 36GB 이하의 릴레이션도 재귀 파티셔닝 없이 처리 가능
