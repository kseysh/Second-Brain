...
FIFO 큐 사용 
###### Belady의 역설이란?
프레임 수를 늘렸지만, 오히려 페이지 폴트가 증가하는 현상
###### OPT 알고리즘이란?
...
장점:
	•	페이지 교체가 필요할 때만 작동하여 오버헤드가 매우 낮음 
단점:
	•	정확도가 높지 않음
...
•	페이지 교체가 필요할 때는 clock scheme을 사용하되, 네 가지 상태 중 가장 낮은 우선순위 그룹에서 교체
...
장점: 처리량이 높아짐
단점: 프로세스 실행 시간이 같이 사용되는 프로세스에 따라 큰 변동이 생길 수 있음
###### Local Allocation과 장단점
