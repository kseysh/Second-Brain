...
turn을 넘겨야 하는 이유: turn을 자신으로 하고 실행하면, 서로 자신 먼저 들어가려고 하므로 누가 먼저 들어가야 할지 결정할 기준이 없음 따라서 turn을 도입해서 누가 먼저 들어갈지 순서를 정하는 장치로 사용 
...
여기서는 v이후 p가 와야 하므로 task1->task2의 순서 동기화가 가능해짐
...
S: 현재 기다리고 있는 프로세스가 몇 개 있는지 ex) s = -2면 2개의 프로세스가 대기 중
