...
![[Pasted image 20250605165037.png|200]]
원래 next victim이 있지만 얘와 얘 아래는 1이라서 0인 다다음애가 victim이 됨
...
•	프로세스가 충분한 수의 페이지(프레임)를 가지지 못하면, 페이지 폴트율이 매우 높아짐
	...
	•	이로 인해 다음과 같은 결과 발생:
		...
		•	새로운 프로세스를 시스템에 추가
•	스래싱(thrashing): 프로세스가 페이지를 들여오고 내보내는 작업에만 몰두하게 되는 현상
![[Pasted image 20250602155000.png|300]]
page fault rate이 갑자기 올라감
...
커널은 page fault가 발생하지 않는 시스템임
•	커널 메모리는 사용자 메모리와는 다르게 처리됨
•	보통 **free-memory pool**에서 할당됨
