...
###### unsafe state
교착 상태로 이어질 수 있는 상태이며 safe state가 아닌 상태
###### signal-and-wait와 signal-and-continue 기법의 차이
프로세스 Q가 x.wait로 중단되어 있고, 프로레스 P가 x.signal을 호출했을 때, signal and wait의 경우 P가 sig_lock_queue에서 대기하고 Q가 실행되며 signal and continue의 경우 Q가 sig_lock_queue에서 대기하고 P가 실행된다.
###### Q
A
###### Q
A
