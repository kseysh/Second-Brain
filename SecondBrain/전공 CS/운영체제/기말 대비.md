...
###### Deadlock이 발생하기 위한 필요조건 4가지
상호 배제, 비선점, 점유 대기, 순환 대기
###### safe state
프로세스에 최대 자원을 할당해도 deadlock이 발생하지 않는 순서가 존재하는 상태
###### unsafe state
deadlock으로 이어질 수 있는 상태이며 safe state가 아닌 상태
###### signal-and-wait와 signal-and-continue 기법의 차이
프로세스 Q가 x.wait()로 중단되어 있고, 프로세스 P가 x.signal()을 호출했을 때, 
signal and wait의 경우 P가 sig_lock_queue에서 대기하고 Q가 실행되며 
signal and continue의 경우 Q가 sig_lock_queue에서 대기하고 P가 실행된다.
###### monitor lock은 1, sig_lock은 0으로 초기화 되어야 하는 이유
모니터는 한 번에 하나의 프로세스만 모니터 내에서 활성일 수 있으므로 monitor_lock을 1로 하여 모니터 내에서 하나의 프로세스만 활성일 수 있도록 하며, 
sig_lock을 0으로 하여 wait(sig_lock)이 실행되어 sig_lock_queue에 프로세스가 한 개 이상 있을 때만 signal(sig_lock)이 호출될 수 있도록 하기 위함
###### Q
A
###### Q
A
