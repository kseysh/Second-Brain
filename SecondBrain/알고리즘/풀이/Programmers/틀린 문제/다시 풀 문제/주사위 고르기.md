...
long long GetWinCount(const vector<int> &a, const vector<int> &b, const vector<vector<int>> &dice)
{
    ...
    // map은 기본적으로 ordered_map이라 an 값에 따라 정렬되므로 an이 bn보다 작은 순간 break를 넣을 수 있는 것임
    for(const auto &[an, ac] : anum) {// an: a의 주사위 합, ac: an이 나오는 경우의 수
...
vector<int> solution(vector<vector<int>> dice) {
    vector<int> own(dice.size(), 1); //주 사위별 소유 플레이어. 0은 A, 1은 B의 소유
    ...
    }while(next_permutation(own.begin(), own.end())); // ?? 이런 함수가 있답니다
    return answer;
}
```
## 알아둘 것
i를 6진법으로 계산하는 테크닉
map을 활용해 시간복잡도를 줄이는 테크닉
next_permutation 함수로 다음 순열로 쉽게 이동하는 STL
ex) {1, 2, 3}의 순열을 123, 132, 213 ... 321로 자동으로 바꿔줌 다음 순열이 존재하지 않으면 false를 return함
따라서 맨 청므 순열을 포함해서 출력하기 위해 do-while문과 함께 사용함.
