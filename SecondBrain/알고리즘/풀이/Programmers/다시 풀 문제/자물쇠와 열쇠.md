...
풀이를 보니 90도 돌린 lockSpace 4개 만드는게 훨씬 쉬운 풀이라는 것이 생각났다...
...
class Solution {
    ...
    public boolean solution(int[][] key, int[][] lock) {
        m = key.length;
        n = lock.length;
        createDuplicatedKey(key);
        return checkAllKeys(lock);
    }
    
    boolean checkAllKeys(int[][] lock){
        
        int emptyCount = 0;
        for(int y = 0; y < n; y++){
            for(int x = 0; x < n; x++){
                if(lock[y][x] == 0) emptyCount++;
            }
        }
        ...
        for(int sy = -n + 1; sy < n; sy++){
            for(int sx = -n + 1; sx < n; sx++){
                for(int d = 0; d < 4; d++){
                    if(checkKey(emptyCount, sy, sx, duplicatedKey[d], lock)){
                        return true;   
                    } 
                }
            }
    ...
    boolean checkKey(int emptyCount, int sy, int sx, int[][] key, int[][] lock){
        int count = 0;
        ...
        for(int ky = 0; ky < m; ky++){
            for(int kx = 0; kx < m; kx++){
                int ly = sy + ky;
                int lx = sx + kx;
                if(ly >= n || lx >= n || ly < 0 || lx < 0) continue;
                if(lock[ly][lx] == 1 && key[ky][kx] == 1) return false;
                if(lock[ly][lx] == 0 && key[ky][kx] == 0) return false;
                if(lock[ly][lx] == 0 && key[ky][kx] == 1) count++;
            }
        }
    ...
    void createDuplicatedKey(int[][] key) {
        
        duplicatedKey = new int[4][m][m];
        
        for(int y = 0; y < m; y++){
            for(int x = 0; x < m; x++){
                duplicatedKey[0][y][x] = key[y][x];
            }
        }
        
        for(int y = 0; y < m; y++){
            for(int x = 0; x < m; x++){
                duplicatedKey[1][x][m - 1 - y] = key[y][x];
            }
        }
        
        for(int y = 0; y < m; y++){
            for(int x = 0; x < m; x++){
                duplicatedKey[2][m - 1 - y][m - 1 - x] = key[y][x];
            }
        }
        
        for(int y = 0; y < m; y++){
            for(int x = 0; x < m; x++){
                duplicatedKey[3][m - 1 - x][y] = key[y][x];
            }
        }
    }
}
```
sy가 -n + 1부터 시작해야한다는 생각을 하지 못함...