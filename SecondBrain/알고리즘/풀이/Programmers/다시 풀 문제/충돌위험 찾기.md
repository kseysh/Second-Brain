...
class Solution {
    ...
    final int MAX_N = 100;
    int[][][] crashCount = new int[MAX_N + 1][MAX_N + 1][200 * MAX_N + 1]; // y, x, time = crashCount
    public int solution(int[][] points, int[][] routes) {
        for(int[] route : routes){
            int time = 0;
            int startIdx = route[0] - 1;
            int[] startPoint = points[startIdx];
            
            crashCount[startPoint[0]][startPoint[1]][time]++;
            
            for(int i = 1; i < route.length; i++){
                startIdx = route[i - 1] - 1;
                int endIdx = route[i] - 1;
                startPoint = points[startIdx];
                int[] endPoint = points[endIdx];
                
                time = move(startPoint[0], startPoint[1], endPoint[0], endPoint[1], time);
            }
        }
        ...
        for(int y = 1; y <= MAX_N; y++){
            for(int x = 1; x <= MAX_N; x++){
                for(int t = 0; t <= 200 * MAX_N; t++){
                    if(crashCount[y][x][t] >= 2) answer++;
                }
            }
        }
        return answer;
    ...
    int move(int sy, int sx, int ey, int ex, int ct){
        while(sy != ey || sx != ex){
            ct++;
            if(sy != ey){
                if(sy > ey) sy--;
...
```
`int[][][] crashCount = new int[MAX_N + 1][MAX_N + 1][200 * MAX_N + 1];` => 너무 오래 걸린다. map<PosAndTime, Integer>ㅣ이게 나을듯

게다가 max time을 200으로 했어서 틀림