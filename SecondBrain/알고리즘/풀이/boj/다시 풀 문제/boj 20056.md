...
```
내가 사용한 map = 이 사람의 q
내가 사용한 sumedFireMap = 이 사람의 v 
항상 두 벡터를 사용하고 하나씩 clear 시키는 방식을 사용했는데 **queue를 사용하니 시간 복잡도와 메모리 효율성이 증가**할 수 있었다. 

q에 파이어볼을 넣어둔다. q는 3차원 벡터이기에 중복된 파이어볼을 체크할 수 있다.
격자를 넘어가는 것에 대해서 `v[(r + dx[d] * s + 10000) % n][(c + dy[d] * s + 10000) % n]`라는 간단한 식을 활용했다.
질량이 0인 파이어볼은 소멸하는데, 이를 `if(m)`으로 간단히 구현했다.
...
## 잘못 푼 이유 분석
파이어볼이 격자를 넘어가도 되는 줄 알고 map으로 접근을 시도함 => 문제를 제대로 이해하지 못함
제대로 된 문제 분석 및 풀이 고민을 더 하지 못하고 구현이 조금 성급했음
